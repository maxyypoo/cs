  ...
  Node<ItemType>* newNodePtr = new Node<ItemType>(newEntry); 
   if (ableToInsert)
   {
     ...
     if (newPosition == 1)
     {
       ...
       headPtr = newNodePtr;
       //newNodePtr -> setNext(newNodePtr -> getNext());
    ...
    //   //cout << "enter";
       newNodePtr -> setNext(prevPtr ->getNext());
         //curPtr = getNodeAt(newPosition - 1);
...
void LinkedList<ItemType>::setEntry(int position, const ItemType& newEntry) throw(PrecondViolatedExcep)
{
 ...
 template <class ItemType>
  ...
  {
    return curPtr;
    // Insert new node at beginning of subchai
  }
  else
  {
    Node<ItemType>* subChainPtr = getNodeAtRecursive(position - 1, subChainPtr->getNext());
    return subChainPtr;
  } // end if
...
Node<ItemType>* LinkedList<ItemType>::getNodeAt(int position) const
{
   ...
   Node<ItemType>* curPtr = getNodeAtRecursive(position, headPtr); 
